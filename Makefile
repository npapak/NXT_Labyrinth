...
# Files
########################################################################
SRC  += display.c Cstartup_SAM7.c aic.c pit.c led.c i2c.c hwinit.c input.c output.c button.c queue.c scheduler.c $(TARGET).c
ASRC  = Cstartup.S 
COBJ  = $(SRC:.c=.o) 
AOBJ  = $(ASRC:.S=.o)
...
LDFLAGS += -L$(PREFIX)/lib/thumb#/interwork -lc -lgcc
LDFLAGS += -T./$(SUBMDL)-$(RUN_MODE).ld -lm
...
download: $(TARGET).bin
	@echo -e '[ $(BBLUE)FWFLASH$(RESET) ]' $<
	@sudo $(FLASH_TOOL) $(TARGET).bin

# Create final output file (.hex) from ELF output file.
%.hex: %.elf
	@echo -e '[ $(BMAGENTA)OBJCOPY$(RESET) ]' $<
	@$(OBJCOPY) -O binary $< $@
...
# testing: option -C
%.lss: %.elf
	@echo -e '[ $(BCYAN)OBJDUMP$(RESET) ]' $<
	@$(OBJDUMP) -h -S -C $< > $@

# Create a symbol table from ELF output file.
%.sym: %.elf
	@echo -e '[ $(BWHITE)NM$(RESET) ]' $<
	@$(NM) -n $< > $@
...
clean:
	@echo -e '[ $(BRED)RM$(RESET) ]' $(TARGET).hex $(TARGET).obj $(TARGET).map  $(TARGET).obj \
	 					$(TARGET).a90 $(TARGET).sym $(TARGET).lnk $(TARGET).lss \
						$(TARGET).rfw \
				 		$(COBJ) $(AOBJ) $(LST) $(SRC:.c=.s) $(SRC:.c=.d) .dep/*
	@$(REMOVE) $(TARGET).hex $(TARGET).obj $(TARGET).map  $(TARGET).obj \
	 					$(TARGET).a90 $(TARGET).sym $(TARGET).lnk $(TARGET).lss \
						$(TARGET).rfw \
				 		$(COBJ) $(AOBJ) $(LST) $(SRC:.c=.s) $(SRC:.c=.d) .dep/*

distclean: clean
	@echo -e '[ $(BRED)RM$(RESET) ]' $(TARGET).bin $(TARGET).elf
	@$(REMOVE) $(TARGET).bin $(TARGET).elf
